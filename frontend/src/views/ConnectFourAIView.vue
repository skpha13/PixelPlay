...
import GlassButton from "@/components/GlassButon.vue";
...
const getAIMove = async () => {
  try {
    const response = await axios.post("http://localhost:8080/connectFour/generateMove", {
      board: board.value,
...
const countDirection = (line: number, column: number, dx: number, dy: number) => {
...
const checkWinForAI = () => {
  for (let row = 0; row < 6; row++) {
    for (let col = 0; col < 7; col++) {
      if (board.value[row][col] === 2 && checkWin(row, col)) {
        return true;
      }
    }
  }
...
const checkTie = () => {
  for (let i = 0; i < 7; ++i) {
    if (board.value[0][i] === 0) return false;
  }
  return true;
};
...
<template>
  <div class="connect-four flex flex-col items-center justify-center">
    <Logo title="Connect Four AI" class="mb-10"/>
    <div class="game-board bg-blue-800 border-blue-800 border-8 inline-block">
      ...
      >
        <div
            ...
            @click="playerTurn === 1 && dropPiece(colIndex)"
            :class="{
            'bg-gray-200': cell === 0,
            'bg-pink-500': cell === 1,
    ...
    <div v-if="winner !== 0" class="winner mt-4">
      <p v-if="winner === 1 || winner === 2" class="text-lg font-bold">
